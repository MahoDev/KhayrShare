...
if (missingFields.length > 0) {
  console.error(
    `FATAL: Missing required configuration fields in config.json: ${missingFields.join(", ")}`,
  );
  console.error(
    "Please refer to src/config/config.example.json for the required structure.",
  );
  process.exit(1);
}
